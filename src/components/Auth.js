...
const Auth = () => {
  // state variable toggling between 'login' and 'signup' modes
  ...
  const handleAuthSubmit = (e) => {
    e.preventDefault();
    // Bhai yahan custom backend pipeline integerate ho jayegi baad me
  ...
  return (
    <section className="agro-auth-section d-flex align-items-center justify-content-center min-vh-100 bg-white">
      <div className="container">
        ...
        {/* Dynamic Inner Dual-Grid Wrapper Card */}
        <div className="row g-0 auth-card mx-auto rounded-4 overflow-hidden shadow-lg">
          
          {/* LEFT COLUMN: BRAND VISUAL ARTWORK DISPLAY (50% WIDTH) */}
          <div className="col-12 col-md-6 position-relative d-none d-md-block art-banner-column">
            <img 
              src="https://i.pinimg.com/736x/17/8d/9b/178d9bd35d762085f2a6e234c9848064.jpg" 
          ...
          {/* RIGHT COLUMN: INTERACTIVE FORM CONTAINER (50% WIDTH) */}
          <div className="col-12 col-md-6 d-flex flex-column justify-content-center p-4 p-lg-5 form-content-column">
            ...
            {/* Main Dynamic Authentication Form */}
            <form className="auth-core-form" onSubmit={handleAuthSubmit}>
              ...
              {/* Conditional Signup Field: Business License Identification */}
              {!isLoginMode && (
              ...
              <button type="submit" className="btn btn-dark w-100 auth-action-btn rounded-pill py-2.5 text-uppercase fw-bold tracking-wider mt-2">
                {isLoginMode ? 'Authenticate Account' : 'Initialize Partnership'}
              </button>

            </form>

            {/* Bottom Form Toggler Option Switch */}
